...
func TestResourceAccessTokenAudience(t *testing.T) {
	...
	accessToken, _, err := s.newAccessToken(ctx, "test-client", claims, []string{"openid"}, "nonce", "conn", time.Time{}, nil, resources)
	require.NoError(t, err)

	idToken, _, err := s.newIDToken(ctx, "test-client", claims, []string{"openid"}, "nonce", accessToken, "", "conn", time.Time{}, nil, nil)
	require.NoError(t, err)
...
func TestResourceAccessTokenMultipleResources(t *testing.T) {
	...
	accessToken, _, err := s.newAccessToken(ctx, "test-client", storage.Claims{UserID: "1"}, []string{"openid"}, "", "conn", time.Time{}, nil, resources)
	require.NoError(t, err)
...
func TestNoResourceAudienceUnchanged(t *testing.T) {
	...
	accessToken, _, err := s.newAccessToken(ctx, "test-client", claims, []string{"openid"}, "nonce", "conn", time.Time{}, nil, nil)
	require.NoError(t, err)
