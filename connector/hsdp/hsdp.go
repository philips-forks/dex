...
type Config struct {
	...
	StateViaCookie   bool      `json:"stateViaCookie"`
...
func (c *Config) Open(id string, logger *slog.Logger) (conn connector.Connector, err error) {
	...
	return &HSDPConnector{
		...
		stateViaCookie:            c.StateViaCookie,
	}, nil
}
...
func (c *HSDPConnector) StateViaCookie() bool {
	return c.stateViaCookie
}
...
func extractAssertion(r *http.Request) string {
	if r == nil {
		return ""
	}
	q := r.URL.Query()
	if assertion := q.Get("assertion"); assertion != "" {
		return assertion
	}
	if samlResp := q.Get("SAMLResponse"); samlResp != "" {
		return samlResp
	}
	_ = r.ParseForm()
	if assertion := r.FormValue("assertion"); assertion != "" {
		return assertion
	}
...
func (c *HSDPConnector) HandleCallback(s connector.Scopes, connData []byte, r *http.Request) (identity connector.Identity, err error) {
	...
	assertion := extractAssertion(r)
	code := q.Get("code")

	// SAML2 / Assertion flow
	if assertion != "" {
		form := url.Values{}
		...
		return c.createIdentity(r.Context(), identity, token, r, createCaller, assertion)
	}
	...
	return identity, errors.New("hsdp: callback request missing both \"code\" and \"assertion\" parameters")
}
...
func (c *HSDPConnector) createIdentity(ctx context.Context, identity connector.Identity, token *oauth2.Token, r *http.Request, caller caller, assertion string) (connector.Identity, error) {
	var claims map[string]interface{}

	cd := ConnectorData{}

	if caller == createCaller && assertion != "" {
